...
type Item struct {
	...
	// History is the history of the Item
	History string

	//Trend is the trend of the Item
	Trend   string

	////Delay is the delay item
	//Delay time.Second
	...
	// PrevValue  is type of PrevValue
	PrevValue string
...
func (c *Session) ItemCreate(params []ItemCreateParams)(resp ItemCreateResponse,err error){
	err = c.Get("item.create",params,&resp)
	if err != nil{
		return resp, err
	}
	return resp, nil
